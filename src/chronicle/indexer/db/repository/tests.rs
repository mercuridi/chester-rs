...
use crate::chronicle::indexer::{Document, scanner};
...
fn resolve_documents(root: &Path, documents: &[Document]) -> Result<LinkResolution> {
    let catalogue = link_resolver::catalogue_from_documents(root, documents)?;
...
async fn resolver_output_persists_body_and_frontmatter_provenance_with_visibility() -> Result<()> {
    ...
    let resolution = resolve_documents(&corpus, &documents)?;
...
async fn excluded_dangling_and_ambiguous_links_persist_no_graph_edge() -> Result<()> {
    ...
    let (documents, _) = scanner::discover_directory_with_stats_excluding(&corpus, &excluded)?;
    let resolution = resolve_documents(&corpus, &documents)?;
...
async fn structured_conditions_query_scalar_and_wikilink_list_metadata() -> Result<()> {
    ...
    let mut plain_target_plan = crate::chronicle::query::StructuredPlan::try_from(
        crate::chronicle::query::parse_plan(
    ...
    let mut literal_plan = crate::chronicle::query::StructuredPlan::try_from(
        crate::chronicle::query::parse_plan(
...
async fn string_or_wikilink_resolution_respects_field_type_and_access_scope() -> Result<()> {
    ...
    let make_plan = |value: &str| -> Result<crate::chronicle::query::StructuredPlan> {
        crate::chronicle::query::StructuredPlan::try_from(crate::chronicle::query::parse_plan(
            &format!(
...
async fn count_members_resolves_note_identifiers_and_deduplicates_values() -> Result<()> {
    ...
    for subject in ["[[Ada]]", "[[ada]]", "[[The Gardener]]"] {
        let plan = crate::chronicle::query::StructuredPlan::try_from(
            crate::chronicle::query::parse_plan(&format!(
